...
# a scheme interpreter which can handle SICP through section 1.1.8.
...
def main():
	parse_args()
	for statement in tokenize_statements(read_source_file("stdlib.scm"), 0):
		evaluate_combination(statement)
	for statement in tokenize_statements(read_source_file(sys.argv[-1]), 0):
...
def define(operands):
	log("define: %s" % operands)
	if type(operands[0]) is str:
		environment[operands[0]] = operands[1]
	else:
		# TODO body needs to support several cases:
		#  1. a single value
		#  2. a procedure body (a list)
		#  3. a list of define calls followed by a procedure body
		# TODO further, sub-definitions need to exist in a sub-namespace
